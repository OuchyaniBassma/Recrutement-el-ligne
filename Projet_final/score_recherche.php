...
<?php
 ...
 $req2=$bdd->prepare('SELECT * FROM scores ORDER BY score DESC');
 $req2->execute();
 if($req2->rowcount()>0){
  ...
  $req4->execute(array($result['num_c']));
  $result4=$req4->fetch();
   ...
   echo '<strong>'.$result2['nom'].'  '.$result2['prenom'].'</strong><br>';
   echo '<input type="submit" name="mssg" value="Envoyer message">';
   echo '<br>';
 ...
 </svg>';echo ' :';echo $result2['email'];echo '<br>';
...
</svg> ';echo ' :';echo $result2['tel'];echo '<br>';
  echo '<input type="submit" name="'.$result['num_c'].'" value="Voir profil">';
